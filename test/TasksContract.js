...
contract("TasksContract", (accounts) => {
  ...
  it("get Tasks List", async () => {
    ...
    assert.equal(task.title, "Example Title");
    assert.equal(task.description, "Example Description");
    assert.equal(task.done, false);
    assert.equal(tasksCounter, 1);
  });

  // can create a task correctly
  ...
  // can change the state of a task correctly
